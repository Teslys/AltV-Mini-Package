...
function invokeCmd(player, cmd, args) {
    ...
    if (callback) {
        ...
        send(player, `{FF0000} /${cmd} Böyle Bir Komut Yok`);
        alt.emit('WrongCommand',player)
    }
}
...
export function unregistercmd(cmd) {
    if(cmdHandlers[cmd] !== undefined) {
        alt.log(cmd + " Yeniden kayıt edildi.")
        cmdHandlers[cmd] = undefined;
    }
}
